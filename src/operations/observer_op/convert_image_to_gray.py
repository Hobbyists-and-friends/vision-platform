...
class ConvertImageToGray(ObserverOpBase):
    ...
    def _update_impl(self, publisher: IPublisher, data: dict) -> None:
        src_image = System.system.variables[self._params[SRC_VARIABLE]].data[VALUE_KEY]
        ...
        if src_image is not None:
            ...
            if RESULT_VARIABLE in self._params:
                ChangeVariableValueOperation(
                    variable_id=self._params[RESULT_VARIABLE], new_value=gray_image
                ).run()
    ...
    def __is_image(self, variable_id: str) -> bool:
        return (
            System.system.variables[variable_id].type == VariableType.IMAGE
            or System.system.variables[variable_id].type == VariableType.NULL
        )
